 ...
 * only limitation is that you cannot save comments when saving the data in a file (for now).
 *
 * @author LoRy24
 */
...
public class YamlConfiguration {
     ...
     *         file doesn't exist
     */
    public YamlConfiguration loadConfig(final File file) throws FileNotFoundException {
    ...
    public YamlConfiguration loadConfig(final String yaml) {
    ...
    public Object get(@NotNull final String path) {
        ...
        for (int i = 0; i < pathKeys.length; i++) if (i != pathKeys.length -1)
            currentHashMap = (HashMap<String, Object>) currentHashMap.get(pathKeys[i]);
     ...
     * in this class. It works using a recursion-based system. The system is very hard to explain so try to figure out
    ...
    private Object internalRecursiveSet(@NotNull final String path,
                                        final Object valueToApply,
                                        final HashMap<String, Object> copy) {
        ...
        // Replace the object in the hashmap with the new hashmap or the object returned by this function. Recursion
     ...
     * @throws IOException If there is an error while opening the FileWriter used to save the data. This can occur if
